...
#define TEAM_NO      16     //insert your team number here
...
// logfilename[--i] = '\0';
#ifdef TM_DEBUG
   // Use logfilename instead of logfile since logfile is a FILE*
...
int zgt_tm::TxWrite(long tid, long obno, int thrNum)
 {
  ...
  #ifdef TM_DEBUG
  printf("\nentering TxWrite for TID: %ld, OBNO: %ld\n", tid, obno);
  fflush(stdout);
#endif

  // Allocate and initialize the parameter structure to pass to the thread.
  struct param *nodeinfo = (struct param*) malloc(sizeof(struct param));
  if (!nodeinfo) {
      fprintf(stderr, "Memory allocation failed in TxWrite\n");
      exit(1);
  }
  nodeinfo->tid = tid;
  nodeinfo->obno = obno;
  nodeinfo->Txtype = ' '; // No extra type info is needed; placeholder.
  nodeinfo->count = --SEQNUM[tid]; // Decrement the sequence number to enforce order.

  // Create a new thread that executes the writetx() function.
 ...
 {
 #ifdef TM_DEBUG
    printf("\nEntering CommitTx for TID: %ld\n", tid);
    fflush(stdout);
 #endif
 
    // Allocate and initialize the parameter structure for the commit operation.
    struct param *nodeinfo = (struct param*) malloc(sizeof(struct param));
    if (!nodeinfo) {
        fprintf(stderr, "Memory allocation failed in CommitTx\n");
        exit(1);
    }
    nodeinfo->tid = tid;
    nodeinfo->Txtype = ' '; // placeholder; no additional type info needed for commit
    // Commit does not involve an object number so we leave obno unchanged
    nodeinfo->count = --SEQNUM[tid];  // enforce sequential ordering
 
    // Create a new thread that executes the committx() function.
    int status = pthread_create(&threadid[thrNum], NULL, committx, (void*) nodeinfo);
    if (status != 0) {
        printf("ERROR: return code from pthread_create() in CommitTx is: %d\n", status);
        exit(-1);
    }
 
 #ifdef TM_DEBUG
    printf("\nLeaving CommitTx for TID: %ld\n", tid);
    fflush(stdout);
 #endif
 ...
 {
 #ifdef TM_DEBUG
    printf("\nEntering AbortTx for TID: %ld\n", tid);
    fflush(stdout);
 #endif
 
    // Allocate and initialize the parameter structure for the abort operation.
    struct param *nodeinfo = (struct param*) malloc(sizeof(struct param));
    if (!nodeinfo) {
        fprintf(stderr, "Memory allocation failed in AbortTx\n");
        exit(1);
    }
    nodeinfo->tid = tid;
    nodeinfo->Txtype = ' '; // placeholder; no extra type info needed for abort
    // Abort does not need an object number so obno remains unset.
    nodeinfo->count = --SEQNUM[tid];  // enforce sequential ordering
 
    // Create a new thread that executes the aborttx() function.
    int status = pthread_create(&threadid[thrNum], NULL, aborttx, (void*) nodeinfo);
    if (status != 0) {
        printf("ERROR: return code from pthread_create() in AbortTx is: %d\n", status);
        exit(-1);
    }
 
 #ifdef TM_DEBUG
    printf("\nLeaving AbortTx for TID: %ld\n", tid);
    fflush(stdout);
 #endif
